...
import { safeLoads, firstPresent, generateOrderNo, parseDate } from '../../utils/helpers';

// ---------------------------------------------------------------------------
// Chinese / English field name aliases
// ---------------------------------------------------------------------------
...
const CLIENT_FIELD_MAP: Record<string, string> = {
  ...
  '联系电话': 'phone',
  '联系人': 'contactPerson',
  '物流': 'logisticsProvider',
  '物流商': 'logisticsProvider',
  '物流电话': 'logisticsPhone',
  '送货电话': 'deliveryPhone',
  '户籍': 'householdRegistration',
  '客户户籍': 'householdRegistration',
  '客户编号': 'clientCode',
  '编号': 'clientCode',
  // snake_case -> camelCase (for mixed input)
...
const INSTALL_ADDRESS_KEYS = ['安装地址', 'address', '地址'];

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

/** Map a record's fields through a field-name dictionary. */
function mapFields<T extends Record<string, unknown>>(
  ...
  for (const [key, val] of Object.entries(data)) {
    // Already a recognised English key -> pass through
    if (fieldMap[key] === undefined && !key.includes('_')) {
      result[key] = val;
    }
  }

  // Apply aliases last so they override any ambiguous English keys
  for (const [key, val] of Object.entries(data)) {
...
/** Extract client fields from an arbitrary input object. */
function extractClientData(data: Record<string, unknown>): Record<string, unknown> {
  const mapped = mapFields(data, CLIENT_FIELD_MAP);

  // If name is missing, try alternative keys (only set if actually provided)
  if (!mapped.name) {
    const found = firstPresent(data, 'name', '名称', '客户', '客户名称', 'customerName');
    if (found) mapped.name = found as string;
  }

  // Remove fields that are not valid Prisma Client fields
  const validFields = ['name', 'brand', 'address', 'phone', 'contactPerson',
...
/** Extract order fields from an arbitrary input object. */
function extractOrderData(data: Record<string, unknown>): Record<string, unknown> {
...
function parseReceiptSpecs(body: Record<string, unknown>) {
  ...
  const source = specs && typeof specs === 'object' ? specs as Record<string, unknown> : {};
  const customerInfoRaw = source.customerInfo ?? source.customer_info ?? source.客户信息 ?? {};
  const customerInfo = customerInfoRaw && typeof customerInfoRaw === 'object' && !Array.isArray(customerInfoRaw)
    ? customerInfoRaw as Record<string, unknown>
    : {};
  const pingHui = asRecordArray(source.ping_hui ?? source.平开);
  const diaoHui = asRecordArray(source.diao_hui ?? source.吊滑);
  return { customerInfo, specs: source, rows: [...pingHui, ...diaoHui], pingHui, diaoHui };
}

function toProgressRow(row: Record<string, unknown>, customerInfo: Record<string, unknown>, orderNo: string, customerName: string | undefined) {
  return {
    ...row,
