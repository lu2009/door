(function () {
  ...
  function hookPrintTemplate() {
    ...
    // ── wrap HTML-returning methods: swap image URLs with cached data URIs ──
    ["getHtml", "getHtmlAsync", "getSimpleHtml", "getSimpleHtmlAsync"].forEach(function (name) {
      ...
      proto[name] = function () {
        ...
        // result may be a string (getHtml) or Promise<string> (getHtmlAsync)
    ...
    // ── wrap action methods: rewrite args only ──
  ...
  /**
   * Replace /api/v1/files/{id} image src in hiprint HTML with cached data URIs.
   */
  ...
  /**
   * Preload images via batch API and cache them (memory + IndexedDB).
   * Call this BEFORE triggering PDF export.
   *
   * @param {string[]} ids  - image IDs to preload
   * @param {string}   ds   - database name (tenant)
   * @returns {Promise<number>} count of successfully cached images
   */
  window.__smartdoor_preloadImages = function (ids, ds) {
    if (!ids || !ids.length) return Promise.resolve(0);

    // 1. Check which are already in memory cache
    var missing = [];
    ...
    // 2. Try IndexedDB for missing ones
  ...
  /**
   * Get a data URI for an image id (from memory cache).
   * Returns null when not cached — caller should preload first.
   */
  ...
  /**
   * Clear all image caches (memory + IndexedDB).
   */
